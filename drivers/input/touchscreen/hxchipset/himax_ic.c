...
#define SZ_SIZE 0x5000
#define TEST_DATA_PATH "/data/vendor/misc/touch/"
...
#ifdef HX_GAP_TEST
	HX_INSPECT_EGAP_RAW = 7 << 1,        /* Sensor short error */
#endif
#ifdef HX_DOZE_TEST
	HX_INSPECT_EDOZE_RAW = 8 << 1,		   /* DOZE RAW ERROR */
	HX_INSPECT_EDOZE_NOISE = 9 << 1,		   /* DOZE NOISE ERROR */
#endif
#ifdef HX_INSPECT_LPWUG_TEST
	HX_INSPECT_ELPWUG_RAW = 10 << 1,		   /* LPWUG RAW ERROR */
	HX_INSPECT_ELPWUG_NOISE = 11 << 1,		   /* LPWUG NOISE ERROR */
	HX_INSPECT_ELPWUG_IDLE_RAW = 12 << 1,		   /* LPWUG IDLE RAW ERROR */
	HX_INSPECT_ELPWUG_IDLE_NOISE = 13 << 1,		   /* LPWUG IDLE NOISE ERROR */
#endif
	HX_INSPECT_EFILE = 14 << 1,					/*Criteria file error*/
	HX_INSPECT_EOTHER = 15 << 1,	/* All other errors */
} HX_INSPECT_ERR_ENUM;
#endif
...
uint32_t mpTestFunc(struct i2c_client *client, uint8_t checktype, uint32_t datalen)
{
	...
	{
	case HIMAX_INSPECTION_OPEN:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_OPEN");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			...
			{
				E("%s: open test FAIL\n", __func__);
				//sprintf(result, "\n OPENMAX = %d, OPENMIN = %d, Result = Failed", OPENMAX, OPENMIN);
				fih_save_test_data(RAW, filepath, result);
				return HX_INSPECT_EOPEN;
			}
		}
		I("%s: open test PASS\n", __func__);
		//sprintf(result, "\n OPENMAX = %d, OPENMIN = %d, Result = Passed", OPENMAX, OPENMIN);
		break;

	case HIMAX_INSPECTION_MICRO_OPEN:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_MICRO_OPEN");
			for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
			{
				if (himax_check_notch(i))
				{
				...
				{
					E("%s: open test FAIL\n", __func__);
					//sprintf(result, "\n M_OPENMAX = %d, M_OPENMIN = %d, Result = Failed", M_OPENMAX, M_OPENMIN);
					fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_EMOPEN;
				}
			}
			I("%s: open test PASS\n", __func__);
			//sprintf(result, "\n M_OPENMAX = %d, M_OPENMIN = %d, Result = Passed", M_OPENMAX, M_OPENMIN);
			break;

	case HIMAX_INSPECTION_SHORT:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_SHORT");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			if (himax_check_notch(i))
			{
			...
			{
				E("%s: short test FAIL\n", __func__);
				//sprintf(result, "\n SHORTMAX = %d, SHORTMIN = %d, Result = Failed", SHORTMAX, SHORTMIN);
				fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_ESHORT;
			}
		}
		I("%s: short test PASS\n", __func__);
		//sprintf(result, "\n SHORTMAX = %d, SHORTMIN = %d, Result = Passed", SHORTMAX, SHORTMIN);
		break;

	case HIMAX_INSPECTION_RAWDATA:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_RAWDATA");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			...
			if ((int)RAW[i] > g_inspection_criteria[IDX_RAWMAX][i] || (int)RAW[i] < g_inspection_criteria[IDX_RAWMIN][i])
			{
				...
				//sprintf(result, "\n RAWMAX = %d, RAWMIN = %d, Result = Failed", RAWMAX, RAWMIN);
				fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_ERAW;
			}
		}
		I("%s: rawdata test PASS\n", __func__);
		//sprintf(result, "\n RAWMAX = %d, RAWMIN = %d, Result = Passed", RAWMAX, RAWMIN);
		break;

	case HIMAX_INSPECTION_NOISE:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_NOISE");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			if (himax_check_notch(i))
			{
			...
			{
				E("%s: noise test FAIL\n", __func__);
				//sprintf(result, "\n NOISEMAX = %d, Result = Failed", NOISEMAX);
				//sprintf(result, "\n NOISEMAX = 200, Result = Failed");
				fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_ENOISE;
			}
		}
		I("%s: noise test PASS\n", __func__);
		//sprintf(result, "\n NOISEMAX = %d, Result = Passed", NOISEMAX);
		//sprintf(result, "\n NOISEMAX = 200, Result = Passed");
		break;
...
#ifdef HX_DOZE_TEST
	case HIMAX_INSPECTION_DOZE_RAWDATA:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_DOZE_RAWDATA");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			if (himax_check_notch(i))
			{
			...
			if ((int)RAW[i] > g_inspection_criteria[IDX_DOZE_RAWDATA_MAX][i] || (int)RAW[i] < g_inspection_criteria[IDX_DOZE_RAWDATA_MIN][i])
			//if (RAW[i] > DOZE_RAWDATA_MAX || RAW[i] < DOZE_RAWDATA_MIN)
			{
				E("%s: HIMAX_INSPECTION_DOZE_RAWDATA FAIL\n", __func__);
				//sprintf(result, "\n DOZE_RAWDATA_MAX = %d, DOZE_RAWDATA_MIN = %d, Result = Failed", DOZE_RAWDATA_MAX, DOZE_RAWDATA_MIN);
				fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_ERAW;
			}
		}
		I("%s: HIMAX_INSPECTION_DOZE_RAWDATA PASS\n", __func__);
		//sprintf(result, "\n DOZE_RAWDATA_MAX = %d, DOZE_RAWDATA_MIN = %d, Result = Passed", DOZE_RAWDATA_MAX, DOZE_RAWDATA_MIN);
		break;
	case HIMAX_INSPECTION_DOZE_NOISE:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_DOZE_NOISE");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			if (himax_check_notch(i))
			{
			...
			if ((int)RAW[i] > g_inspection_criteria[IDX_DOZE_NOISE_MAX][i] || (int)RAW[i] < g_inspection_criteria[IDX_DOZE_NOISE_MIN][i])
			//if (RAW[i] > DOZE_NOISE_MAX || RAW[i] < DOZE_NOISE_MIN)
			{
				E("%s: HIMAX_INSPECTION_DOZE_NOISE FAIL\n", __func__);
				//sprintf(result, "\n DOZE_NOISE_MAX = %d, DOZE_NOISE_MIN = %d, Result = Failed", DOZE_NOISE_MAX, DOZE_NOISE_MIN);
				fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_ENOISE;
			}
		}
		I("%s: HIMAX_INSPECTION_DOZE_NOISE PASS\n", __func__);
		//sprintf(result, "\n DOZE_NOISE_MAX = %d, DOZE_NOISE_MIN = %d, Result = Passed", DOZE_NOISE_MAX, DOZE_NOISE_MIN);
		break;
#endif
#ifdef HX_INSPECT_LPWUG_TEST
	case HIMAX_INSPECTION_LPWUG_RAWDATA:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_LPWUG_RAWDATA");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			if (himax_check_notch(i))
			{
			...
			//if (RAW[i] > LPWUG_RAWDATA_MAX || RAW[i] < LPWUG_RAWDATA_MIN)
			{
				...
				//sprintf(result, "\n LPWUG_RAWDATA_MAX = %d, LPWUG_RAWDATA_MIN = %d, Result = Failed", LPWUG_RAWDATA_MAX, LPWUG_RAWDATA_MIN);
				fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_ERAW;
			}
		}
		I("%s: HIMAX_INSPECTION_LPWUG_RAWDATA PASS\n", __func__);
		//sprintf(result, "\n LPWUG_RAWDATA_MAX = %d, LPWUG_RAWDATA_MIN = %d, Result = Passed", LPWUG_RAWDATA_MAX, LPWUG_RAWDATA_MIN);
		break;
	case HIMAX_INSPECTION_LPWUG_NOISE:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_LPWUG_NOISE");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			if (himax_check_notch(i))
			{
			...
			if ((int)RAW[i] > g_inspection_criteria[IDX_LPWUG_NOISE_MAX][i] || (int)RAW[i] < g_inspection_criteria[IDX_LPWUG_NOISE_MIN][i])
			{
				...
				//sprintf(result, "\n LPWUG_NOISE_MAX = %d, LPWUG_NOISE_MIN = %d, Result = Failed", LPWUG_NOISE_MAX, LPWUG_NOISE_MIN);
				fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_ENOISE;
			}
		}
		I("%s: HIMAX_INSPECTION_LPWUG_NOISE PASS\n", __func__);
		//sprintf(result, "\n LPWUG_NOISE_MAX = %d, LPWUG_NOISE_MIN = %d, Result = Passed", LPWUG_NOISE_MAX, LPWUG_NOISE_MIN);
		break;
	case HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			if (himax_check_notch(i))
			{
			...
			//if (RAW[i] > LPWUG_IDLE_RAWDATA_MAX || RAW[i] < LPWUG_IDLE_RAWDATA_MIN)
			{
				...
				//sprintf(result, "\n LPWUG_IDLE_RAWDATA_MAX = %d, LPWUG_IDLE_RAWDATA_MIN = %d, Result = Failed", LPWUG_IDLE_RAWDATA_MAX, LPWUG_IDLE_RAWDATA_MIN);
				fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_ERAW;
			}
		}
		I("%s: HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA PASS\n", __func__);
		//sprintf(result, "\n LPWUG_IDLE_RAWDATA_MAX = %d, LPWUG_IDLE_RAWDATA_MIN = %d, Result = Passed", LPWUG_IDLE_RAWDATA_MAX, LPWUG_IDLE_RAWDATA_MIN);
		break;
	case HIMAX_INSPECTION_LPWUG_IDLE_NOISE:
		sprintf(filepath, "%s%s", TEST_DATA_PATH, "HIMAX_INSPECTION_LPWUG_IDLE_NOISE");
		for (i = 0; i < (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM); i++)
		{
			if (himax_check_notch(i))
			{
			...
			if ((int)RAW[i] > g_inspection_criteria[IDX_LPWUG_IDLE_NOISE_MAX][i] || (int)RAW[i] < g_inspection_criteria[IDX_LPWUG_IDLE_NOISE_MIN][i])
			{
				...
				//sprintf(result, "\n LPWUG_IDLE_NOISE_MAX = %d, LPWUG_IDLE_NOISE_MIN = %d, Result = Failed", LPWUG_IDLE_NOISE_MAX, LPWUG_IDLE_NOISE_MIN);
				fih_save_test_data(RAW, filepath, result);
					return HX_INSPECT_ENOISE;
			}
		}
		I("%s: HIMAX_INSPECTION_LPWUG_IDLE_NOISE PASS\n", __func__);
		//sprintf(result, "\n LPWUG_IDLE_NOISE_MAX = %d, LPWUG_IDLE_NOISE_MIN = %d, Result = Passed", LPWUG_IDLE_NOISE_MAX, LPWUG_IDLE_NOISE_MIN);
		break;
#endif
	default:
		E("Wrong type=%d\n",checktype);
...
int himax_get_criteria_size(void)
{
	...
	criteria_size = HX_CRITERIA_SIZE;

	return criteria_size;
}
...
int hx_diff_str(char *str1, char *str2)
{
	...
	if(str1_len != str2_len) {
		I("%s:Size different! count_type:%d , result:%d \n", __func__, str1_len, str2_len);
		return -1;
	}
...
int himax_parse_criteria_file(void)
{
	...
	switch(get_Display_ID())
	{
		case DISPLAY_PANEL_ID_HX83112A_TIANMA_FHD_VIDEO:
			file_name = "hx_TM_criteria.csv";
			break;
		case DISPLAY_PANEL_ID_HX83112A_CTC_FHD_VIDEO:
			file_name = "hx_CTC_criteria.csv";
			break;
...
int fih_save_test_data(uint32_t RAW[], char *filepath, char *result)
{
	...
	mm_segment_t fs;
	loff_t pos = 0;
	testdata = kzalloc(SZ_SIZE,GFP_KERNEL);

	if (!testdata) {
		E("%s: Kzalloc error for testdata!\n", __func__);
		return -EIO;
	}
	if(RawdataFile == NULL)
		RawdataFile = filp_open(filepath, O_TRUNC|O_CREAT|O_RDWR, 0660);
	...
	fs = get_fs();
	set_fs(get_ds());
	vfs_write(RawdataFile, testdata, strlen(testdata), &pos);
	if(RawdataFile != NULL)
	{
...
int himax_chip_self_test(struct i2c_client *client)
{
	...
	//1. Open Test
	I("[MP_OPEN_TEST_RAW]\n");
	ret = ret | mpTestFunc(client, HIMAX_INSPECTION_OPEN, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("1. Open Test: End %d\n\n\n", ret);

	//2. Micro-Open Test
	I("[MP_MICRO_OPEN_TEST_RAW]\n");
	ret = ret | mpTestFunc(client, HIMAX_INSPECTION_MICRO_OPEN, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("2. Micro Open Test: End %d\n\n\n", ret);

	//3. Short Test
	I("[MP_SHORT_TEST_RAW]\n");
	ret = ret | mpTestFunc(client, HIMAX_INSPECTION_SHORT, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("3. Short Test: End %d\n\n\n", ret);
			
	//4. RawData Test
	I("==========================================\n");
	I("[MP_RAW_TEST_RAW]\n");
	ret = ret | mpTestFunc(client, HIMAX_INSPECTION_RAWDATA, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM)+ic_data->HX_TX_NUM+ic_data->HX_RX_NUM);
	I("4. Raw Test: End %d\n\n\n", ret);
	
	//5. Noise Test
	I("[MP_NOISE_TEST_RAW]\n");
	ret = ret | mpTestFunc(client, HIMAX_INSPECTION_NOISE, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	...
	//6. DOZE RAWDATA
	//I("[MP_DOZE_TEST_RAW]\n");
	//ret = ret | mpTestFunc(client, HIMAX_INSPECTION_DOZE_RAWDATA, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	//I("6. MP_DOZE_TEST_RAW: End %d\n\n\n", ret);
	
	//7. DOZE NOISE
	//I("[MP_DOZE_TEST_NOISE]\n");
	//ret =ret |  mpTestFunc(client, HIMAX_INSPECTION_DOZE_NOISE, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	...
	//8. LPWUG RAWDATA
	//I("[MP_LPWUG_TEST_RAW]\n");
	//ret = ret | mpTestFunc(client, HIMAX_INSPECTION_LPWUG_RAWDATA, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	//I("8. MP_LPWUG_TEST_RAW: End %d\n\n\n", ret);
	
	//9. LPWUG NOISE
	//I("[MP_LPWUG_TEST_NOISE]\n");
	//ret = ret | mpTestFunc(client, HIMAX_INSPECTION_LPWUG_NOISE, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	//I("9. MP_LPWUG_TEST_NOISE: End %d\n\n\n", ret);
	
	//10. LPWUG IDLE RAWDATA
	//I("[MP_LPWUG_IDLE_TEST_RAW]\n");
	//ret = ret | mpTestFunc(client, HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	//I("10. MP_LPWUG_IDLE_TEST_RAW: End %d\n\n\n", ret);
	
	//11. LPWUG IDLE RAWDATA
	//I("[MP_LPWUG_IDLE_TEST_NOISE]\n");
	//ret = ret | mpTestFunc(client, HIMAX_INSPECTION_LPWUG_IDLE_NOISE, (ic_data->HX_TX_NUM*ic_data->HX_RX_NUM) + ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
...
void himax_read_FW_ver(struct i2c_client *client)
{
	...
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
    uint8_t data[64],data_2[64],data_3[64];
	int retry = 200;
    int reload_status = 0;
	...
	while(reload_status == 0)
	{
		...
		cmd_2[1] = 0x00;
		cmd_2[0] = 0xA8;
		himax_register_read(client, cmd_2, 4, data_3, false);
		I("%s : 900000A8 :data_3[0]=0x%2.2X,data_3[1]=0x%2.2X,data_3[2]=0x%2.2X,data_3[3]=0x%2.2X\n",__func__,data_3[0],data_3[1],data_3[2],data_3[3]);
		...
		cmd_2[0] = 0xE8;
		himax_register_read(client, cmd_2, 4, data_3, false);
		I("%s : 900000E8 :data_3[0]=0x%2.2X,data_3[1]=0x%2.2X,data_3[2]=0x%2.2X,data_3[3]=0x%2.2X\n",__func__,data_3[0],data_3[1],data_3[2],data_3[3]);
		...
		cmd_2[0] = 0x40;
		himax_register_read(client, cmd_2, 4, data_3, false);
		I("%s : 10007F40 :data_3[0]=0x%2.2X,data_3[1]=0x%2.2X,data_3[2]=0x%2.2X,data_3[3]=0x%2.2X\n",__func__,data_3[0],data_3[1],data_3[2],data_3[3]);
		...
		else
		{
			...
			if (retry % 10 == 0)
				...
				I("%s : data[0]=0x%2.2X,data[1]=0x%2.2X,data[2]=0x%2.2X,data[3]=0x%2.2X\n",__func__,data[0],data[1],data[2],data[3]);
				I("%s : data_2[0]=0x%2.2X,data_2[1]=0x%2.2X,data_2[2]=0x%2.2X,data_2[3]=0x%2.2X\n",__func__,data_2[0],data_2[1],data_2[2],data_2[3]);
				tmp_addr[3] = 0x90; tmp_addr[2] = 0x00; tmp_addr[1] = 0x00; tmp_addr[0] = 0xA8;
				himax_register_read(client, tmp_addr, 4, tmp_data, false);
				I("%s: 0x900000A8, tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x \n", __func__, tmp_data[0],tmp_data[1],tmp_data[2],tmp_data[3]);
...
// Product ID
        ...
        if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x11) && ((tmp_data[1] == 0x2a)||(tmp_data[1] == 0x2b)))
        {
            ...
            CFG_VER_MAJ_FLASH_ADDR 	= 49410;  //0x00C102
            CFG_VER_MAJ_FLASH_LENG 	= 1;
            CFG_VER_MIN_FLASH_ADDR 	= 49411;  //0x00C103
...
bool himax_calculateChecksum(struct i2c_client *client, bool change_iref)
{
	...
	I("%s: Now CRC=%d\n", __func__, CRC_result);
	if(CRC_result != 0)
		I("%s: CRC fail, need to Force Update FW!\n", __func__);
	msleep(50);
...
bool himax_flash_lastdata_check(struct i2c_client *client)
{
	...
	{
			I("%s: Fail! flash_buffer[FFFC]=0x%2X,flash_buffer[FFFD]=0x%2X,flash_buffer[FFFE]=0x%2X,flash_buffer[FFFF]=0x%2X\n", __func__,
			...
			I("%s: Need to Force update!\n", __func__);
			return 1;//FAIL
	} else {
			I("%s: Pass! flash_buffer[FFFC]=0x%2X,flash_buffer[FFFD]=0x%2X,flash_buffer[FFFE]=0x%2X,flash_buffer[FFFF]=0x%2X\n", __func__,
				flash_tmp_buffer[flash_page_len-4],flash_tmp_buffer[flash_page_len-3],flash_tmp_buffer[flash_page_len-2],flash_tmp_buffer[flash_page_len-1]);
			return 0;//PASS
	}
}
...
void himax_firmware_update_0f(const struct firmware *fw_entry)
{
	...
	himax_sense_off(private_ts->client, false);
