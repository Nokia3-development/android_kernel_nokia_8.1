...
#ifdef CONFIG_FIH_DUMP_WAKELOCK
#define MAX_EHCS_WS_NAME 64

struct ehcs_ws {
		bool force_update;
		char name[MAX_EHCS_WS_NAME];
		ktime_t long_active_time;
};

static struct ehcs_ws ehcs_ws_data;
static int print_active_wakeup_source_stats(struct wakeup_source *ws)
{
	...
	if (ws->active) {
		...
		/* Find longest wakeup source for EHCS. */
		if (ehcs_ws_data.force_update) {
			ehcs_ws_data.long_active_time = active_time;
			scnprintf(ehcs_ws_data.name, MAX_EHCS_WS_NAME, "%s", ws->name);

			ehcs_ws_data.force_update = false;

		} else if (ktime_compare(active_time, (ehcs_ws_data.long_active_time)) > 0) {
			ehcs_ws_data.long_active_time = active_time;
			scnprintf(ehcs_ws_data.name, MAX_EHCS_WS_NAME, "%s", ws->name);
		}
	}
...
static void print_active_locks(void)
{
	...
	ehcs_ws_data.force_update = true;
	...
	/* Output name and time of LWS(Longest Wakeup Source) if wakelock time over 10 min. */
	if (ktime_to_ms(ehcs_ws_data.long_active_time) > 600000)
		pr_info("BBox::EHCS;55602:i:LWS(%s, %lld)\n",
			ehcs_ws_data.name,
			ktime_to_ms(ehcs_ws_data.long_active_time));
}
